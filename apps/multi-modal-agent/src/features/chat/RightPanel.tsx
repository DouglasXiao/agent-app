...
export const RightPanel: React.FC<RightPanelProps> = ({ activeComponent, componentData, onClose }) => {
    ...
    <div className="h-full bg-white">
      {activeComponent && (
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          ...
          <button onClick={onClose} className="h-8 w-8 text-gray-500 hover:text-gray-700">
            ✕
          </button>
        </div>
      )}

      <div className="h-full overflow-y-auto">{renderComponent()}</div>
    </div>
  );
};
